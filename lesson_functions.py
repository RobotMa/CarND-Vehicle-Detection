...
# A function that generate multiple windows of different sizes
def multiple_windows(image, xy_window_list, x_start_stop_list,
                    y_start_stop_list, xy_overlap_list):
    windows = []
    for x_start_stop, y_start_stop, xy_window, xy_overlap in zip(x_start_stop_list,
            y_start_stop_list, xy_window_list, xy_overlap_list):

        windows = windows + slide_window(image, x_start_stop=x_start_stop,
                y_start_stop=y_start_stop, xy_window=xy_window,
                xy_overlap=xy_overlap)

    return windows


# A function that receives an image
...
def search_windows(img, windows, clf, scaler, color_space='RGB',
    ...
    # Use this if clf is trained with .png while the input is .jpg
...
def find_cars(img, y_start_stop, scale, svc, X_scaler, orient, pix_per_cell, cell_per_block, spatial_size, hist_bins, color_space):
    ...
    for xb in range(nxsteps):
        for yb in range(nysteps):
            ...
            # Convert color to LUV for spatial and hist features' best performance
            subimg = convert_color(subimg, 'LUV')
